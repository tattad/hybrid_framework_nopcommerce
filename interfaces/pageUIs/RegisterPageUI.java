package pageUIs;

public class RegisterPageUI {
    //Tránh việc lặp lại
    //Define 1 lần duy nhất ở đây = không trùng lặp chỗ khác
    //Màn hình nào quản lí element riêng của màn hình đó
    public static final String FIRSTNAME_TEXTBOX = "//input[@id='FirstName']";
    public static final String LASTNAME_TEXTBOX = "//input[@id='LastName']";
    public static final String EMAIL_TEXTBOX = "//input[@id='Email']";
    public static final String PASSWORD_TEXTBOX = "//input[@id='Password']";
    public static final String CONFIR_MPASSWORD_TEXTBOX = "//input[@id='ConfirmPassword']";
    public static final String REGISTER_BTN = "//button[@id='register-button']";
    public static final String FIRSTNAME_ERROR_MSG = "//span[@id='FirstName-error']";
    public static final String LASTNAME_ERROR_MSG = "//span[@id='LastName-error']";
    public static final String EMAIL_ERROR_MSG = "//span[@id='Email-error']";
    public static final String CONFIRM_EMAIL_ERROR_MSG = "//span[@id='ConfirmPassword-error']";
    public static final String REGISTRATION_COMPLETED_MSG = "//div[@class='result']";
}
